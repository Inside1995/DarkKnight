...
public class Knight implements Comparable<Knight> {
    ...
    public Knight(Long id, String name) {
        this.id = id;
        this.name = name;
    }
    ...
    public void train(int needToTrain, int conditionValue, int price, TrainingType trainingType) throws GoldNotEnoughException, ConditionNotEnoughException, TrainingNotPossible {
        ...
        if (goldAmount - price < 0)
            throw new GoldNotEnoughException("У вас недостаточно золота!");
...
//        }
        ...
        if (this.condition.getCondition() - conditionValue < 0)
            throw new ConditionNotEnoughException("У персонажа недостаточно кондиции. Пожалуйста дайте ему отдохнуть и повторите Вашу попытку!");
...
//        }
    ...
    public static Knight initializeKnight(String name) {
        Knight knight = new Knight();
        knight.setName(name);
        knight.setLvl(1);
        knight.setHealth(100);
        knight.setSkills(new Skills());
        return knight;
    }

    public Statistic initializeStatisticForKnight() {
        Statistic statistic = new Statistic();
        statistic.setKnight(this);
        statistic.setWinPercentage(new BigDecimal(0));
        this.setStatistic(statistic);
        return statistic;
    }

    public Condition initializeOrUpdateConditionForKnight() {
        if (this.condition == null) {
            Condition condition = new Condition();
            condition.setCondition(100);
            condition.setLastUpdateTime(new Date());
            this.setCondition(condition);
            condition.setKnight(this);
        } else {
            Date now = new Date();
